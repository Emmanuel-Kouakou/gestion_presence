...
<?php	require_once("../includes/entete2.php") ?>
 ...
 <div class="container espace col-md-6 col-md-offset-3 col-xs-12">
	<div class="panel panel-default">
		...
		<div class="panel-default coul"> AUTHENTIFICATION</div>
...
<?php
      ...
      if(isset($_POST['login']) && !empty($_POST['username']) && !empty($_POST['password'])){
		...
		try {
		 ...
		 $req = "SELECT * FROM users WHERE nom_utilisateur=? AND mot_de_passe=?";
		 $ps= $pdo->prepare($req);
		 ...
		 if ($user=$ps->fetch()) {
		   ...
		   header("location:reconnaissance/index.php");  
			exit();
			...
			//if($ip=='105.235.111.211'){
				...
				#header("location:emarge_user.php?id=$id");
				// header("location:reconnaissance/index.php?id=$id");
				// exit();
			//}else{
				?>
	...
	<div class="alert alert-warning container espace col-md-6 col-md-offset-3 col-xs-12">
      <strong>Impossible de se connecter!</strong> Verifier nom d'utilisateur et/ou votre mot de passe.
    </div>
